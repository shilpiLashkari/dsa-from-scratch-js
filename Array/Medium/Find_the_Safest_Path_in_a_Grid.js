...
var maximumSafenessFactor = function (grid) {
    ...
    const safetyDistance = Array.from({ length: rows }, () => Array(cols).fill(-1));
    const queue = [];
    ...
    while (frontier.length > 0) {
        ...
        for (const [dr, dc] of directions) {
            ...
            if (
                ...
                const nextSafety = Math.min(currentSafety, safetyDistance[nextRow][nextCol]);
...
console.log('Test 1:', maximumSafenessFactor(grid1)); // Expected: 1
...
console.log('Test 2:', maximumSafenessFactor(grid2)); // Expected: 0
...
console.log('Test 3:', maximumSafenessFactor(grid3)); // Expected: 1
