// Problem: Sign of the Product of an Array
// There is a function signFunc(x) that returns:
// 1 if x is positive.
// -1 if x is negative.
// 0 if x is equal to 0.
// You are given an integer array nums. Let product be the product of all values in the array nums.
// Return signFunc(product).

// Example 1:
// Input: nums = [-1,-2,-3,-4,3,2,1]
// Output: 1
// Explanation: The product of all values in the array is 144, and signFunc(144) = 1

// Example 2:
// Input: nums = [1,5,0,2,-3]
// Output: 0
// Explanation: The product of all values in the array is 0, and signFunc(0) = 0

// Constraints:
// 1 <= nums.length <= 1000
// -100 <= nums[i] <= 100

/**
 * @param {number[]} nums
 * @return {number}
 */
var arraySign = function(nums) {
    let negativesCount = 0;
    
    for (let num of nums) {
        if (num === 0) {
            return 0; // If any number is 0, the product is 0.
        }
        if (num < 0) {
            negativesCount++;
        }
    }
    
    // If the count of negative numbers is even, the product is positive.
    // If it's odd, the product is negative.
    return (negativesCount % 2 === 0) ? 1 : -1;
};

// Notes:
// - We don't need to actually calculate the product, which could cause integer overflow.
// - We only care about the sign.
// - A single zero makes the entire product zero.
// - An odd number of negatives makes the product negative; an even number makes it positive.
// - Time Complexity: O(N)
// - Space Complexity: O(1)

module.exports = { arraySign };
